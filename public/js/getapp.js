...
const META = [
  ...
  { re: /cli-linux$/, group: "Terminal edition (CLI)", os: "Linux", fmt: "CLI binary", cmd: (f, u) => `curl -L ${u} -o darknode && chmod +x darknode && ./darknode` },
  { re: /cli-windows\.exe$/i, group: "Terminal edition (CLI)", os: "Windows", fmt: "CLI .exe", cmd: (f, u) => `curl.exe -L ${u} -o darknode.exe; .\\darknode.exe` },
  { re: /\.dmg$/, group: "Desktop app", os: "macOS", fmt: ".dmg (Apple Silicon + Intel)", cmd: (f) => `open ${f}   # then drag Darknode to Applications` },
  { re: /-macos-arm64$/, group: "Terminal edition (CLI)", os: "macOS · Apple Silicon", fmt: "CLI binary", cmd: (f, u) => `curl -L ${u} -o darknode && chmod +x darknode && codesign --sign - darknode && ./darknode` },
  { re: /-macos-x64$/, group: "Terminal edition (CLI)", os: "macOS · Intel", fmt: "CLI binary", cmd: (f, u) => `curl -L ${u} -o darknode && chmod +x darknode && codesign --sign - darknode && ./darknode` },
];
  ...
  main.innerHTML = `
    ...
    <div class="card dl-hero" style="border-color:color-mix(in srgb,var(--acc) 45%,transparent)">
      ...
      <div class="btns" style="flex-wrap:wrap;gap:8px;margin-bottom:12px">
        <a class="btn" href="${OS_REPO}" target="_blank" rel="noopener">Get Darknode OS &middot; GitHub</a>
      ...
      <div style="margin-top:12px;display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:8px;font-size:.78rem">
        <div style="background:var(--card2);border:1px solid var(--line);border-radius:4px;padding:10px">
          <div style="font-weight:600;margin-bottom:4px">Linux (native)</div>
          <code data-cmd style="font-size:.7rem">git clone ${OS_REPO} &amp;&amp; cd darknode-os</code><br>
          <code data-cmd style="font-size:.7rem">./build.sh debian full</code><br>
          <span class="muted">Run with QEMU/KVM or import .qcow2 into VirtualBox</span>
        </div>
        <div style="background:var(--card2);border:1px solid var(--line);border-radius:4px;padding:10px">
          <div style="font-weight:600;margin-bottom:4px">Windows</div>
          <span class="muted">1. Install <a href="https://www.virtualbox.org/wiki/Downloads" target="_blank" rel="noopener">VirtualBox</a></span><br>
          <span class="muted">2. Clone repo &amp; build, or download pre-built .ova</span><br>
          <code data-cmd style="font-size:.7rem">./export-vbox.sh full</code><br>
          <span class="muted">3. Import .ova into VirtualBox</span>
        </div>
        <div style="background:var(--card2);border:1px solid var(--line);border-radius:4px;padding:10px">
          <div style="font-weight:600;margin-bottom:4px">macOS</div>
          <code data-cmd style="font-size:.7rem">brew install qemu</code><br>
          <span class="muted">Clone repo, build, then:</span><br>
          <code data-cmd style="font-size:.7rem">./launch.sh full</code><br>
          <span class="muted">Or import .qcow2 into UTM / VirtualBox</span>
        </div>
      </div>
    </div>
    </div>
...
const APP_DOCS = [
  ...
  { title: "Desktop app — macOS (.dmg)", os: "macOS", fmt: ".dmg · Apple Silicon + Intel",
    ...
    steps: [ { t: "Download the .dmg from the Builds tab, open it, and drag Darknode to Applications:", cmd: `open Darknode-*.dmg` }, { t: "First launch: right-click the app → Open (unsigned build) to clear Gatekeeper once." } ],
    note: "Built with <code>npm run dist:mac</code> from source on a Mac if a signed release isn't posted yet.",
    trouble: "\"App is damaged / can't be opened\" (Gatekeeper on an unsigned build): <code>xattr -dr com.apple.quarantine /Applications/Darknode.app</code>, then open it." },
];
...
const CLI_DOCS = [
  ...
  { title: "Nexus / CLI — from source (git clone)", os: "Any", fmt: "~300 KB + Node",
    ...
    steps: [ { t: "Clone and run:", cmd: `git clone https://github.com/Darknode-Official/darknode-cli && cd darknode-cli && node darknode.js nexus --tui` } ],
