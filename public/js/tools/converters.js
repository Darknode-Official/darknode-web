...
export const TOOLS = [
  ...
  { id: "c-chmod", name: "Unix chmod (Octal ↔ Symbolic)", cat: "converters", desc: "Convert Unix file permissions between octal and symbolic rwx notation.", tags: ["chmod", "unix", "permissions"],
    ...
    run(v) { const s = String(v.value || "").trim(); if (!s) return ""; if (v.mode === "Octal → Symbolic") { if (!/^[0-7]{3,4}$/.test(s)) return { error: "Enter a valid octal mode (e.g. 755 or 0755)." }; const special = s.length === 4 ? +s[0] : 0; const digits = s.length === 4 ? s.slice(1) : s; const base = digits.split("").map((d) => { const n = +d; return (n & 4 ? "r" : "-") + (n & 2 ? "w" : "-") + (n & 1 ? "x" : "-"); }); const applySpecial = (triad, bit, lower, upper) => { if (!bit) return triad; const a = triad.split(""); a[2] = a[2] === "x" ? lower : upper; return a.join(""); }; base[0] = applySpecial(base[0], special & 4, "s", "S"); base[1] = applySpecial(base[1], special & 2, "s", "S"); base[2] = applySpecial(base[2], special & 1, "t", "T"); return base.join(""); } if (!/^([r-][w-][xstST-]){3}$/.test(s)) return { error: "Enter symbolic permissions (e.g. rwxr-xr-x)." }; const groups = [s.slice(0, 3), s.slice(3, 6), s.slice(6, 9)]; let special = 0; const perms = groups.map((g, i) => { let n = 0; if (g[0] === "r") n += 4; if (g[1] === "w") n += 2; const e = g[2]; if (e === "x" || e === "s" || e === "t") n += 1; if ((e === "s" || e === "S") && i === 0) special += 4; if ((e === "s" || e === "S") && i === 1) special += 2; if ((e === "t" || e === "T") && i === 2) special += 1; return n; }).join(""); return special ? String(special) + perms : perms; } },
