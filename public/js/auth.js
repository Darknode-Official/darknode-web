...
import { consoleHTML, directoryHTML, wireConsole, labelOf as navLabel } from "/js/console-nav.js?v=20260925g";
...
const SIDE_DESC = {
  ...
  learn: "Guided lessons — Learn Hub.",
...
function renderApp(user) {
  ...
  function show(sec, more, skipPush) {
    ...
    else if (sec === "admin") { import("/js/admin.js?v=20260925g").then(m => m.renderAdmin(main, user)); }
    ...
    else if (sec === "toolbox") { import("/js/mini-tools.js?v=20260925g").then(m => m.renderToolbox(main, { onOpen: (id) => show("tool-" + id) })); }
    else if (sec.indexOf("tool-") === 0) { const _tid = sec.slice(5); import("/js/mini-tools.js?v=20260925g").then(m => m.renderMiniTool(main, _tid, { onBack: () => show("toolbox") })); }
