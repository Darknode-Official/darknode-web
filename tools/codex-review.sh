...
- Static Firebase-hosted site: public/ is served as-is. ES modules are imported with ?v= cache tags; a changed module needs its tag bumped in every importer, up the chain to index.html.
  Exception: public/quelvra/ uses no ?v= tags on purpose; its service worker (public/quelvra/sw.js) is network-first with cache: no-cache, and its file list/VERSION are regenerated by tools/quelvra-sw-files.mjs. Only flag Quelvra caching if a new file is missing from sw.js FILES.
