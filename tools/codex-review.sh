...
codex exec --sandbox read-only --ephemeral -o "$out" \
  "You are reviewing code another agent wrote. Review $what in this repository. The full diff is below. If your shell works, read surrounding code as needed; do not modify any files.
