...
git -C "$SRC" archive HEAD public/quelvra test/quelvra tools/quelvra-sw-files.mjs $(git -C "$SRC" ls-files 'tools/quelvra-coverage*.mjs') | tar -x -C "$TMP"
...
sed -i 's#\.\./\.\./tools/quelvra-coverage#../tools/coverage#g' test/*.js
...
# the coverage tools live in this repo's tools/ as well
for f in "$TMP"/tools/quelvra-coverage*.mjs; do [ -e "$f" ] && sed 's#\.\./public/quelvra/#../public/#g; s#\./quelvra-coverage#./coverage#g; s#tools/quelvra-coverage#tools/coverage#g' "$f" > "tools/$(basename "$f" | sed 's/^quelvra-//')"; done
